...
define('QA_VERSION', '7.3'); // also used as suffix for .js and .css requests
define('QA_BUILD_DATE', '2022-04-01');
 ...
 */
{
    ...
    echo 'King-Media fatal error:<p><font color="red">' . qa_html($message, true) . '</font></p>';
    @error_log('PHP King-Media fatal error: ' . $message);
    echo '<p>Stack trace:<p>';

    $backtrace = array_reverse(array_slice(debug_backtrace(), 1));
    foreach ($backtrace as $trace) {
        echo '<font color="#' . ((strpos(@$trace['file'], '/king-plugin/') !== false) ? 'f00' : '999') . '">' .

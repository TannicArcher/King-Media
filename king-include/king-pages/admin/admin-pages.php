		...
		if (!qa_check_form_security_code('admin/pages', qa_post_text('code')))
			...
			//	Perform appropriate database action
				...
				if (isset($editpage['pageid'])) { // changing existing page
					...
					if (empty($errors)) {
						if ($isexternal)
							$pageid=qa_db_page_create($inname, $inicon, QA_PAGE_FLAGS_EXTERNAL | ($innewwindow ? QA_PAGE_FLAGS_NEW_WINDOW : 0), $inurl, null, null, $inpermit);
						else {
							$pageid=qa_db_page_create($inname, $inicon, 0, $inslug, $inheading, $incontent, $inpermit);

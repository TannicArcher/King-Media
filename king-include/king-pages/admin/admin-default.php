	...
	if (qa_clicked('doresetoptions')) {
		...
		//	Uploading default avatar
			...
			if (is_array(@$_FILES['logo_file']) && $_FILES['logo_file']['size']) {
				$ImageName = str_replace( ' ', '-', strtolower( $_FILES['logo_file']['name'] ) );
				$TempSrc   = $_FILES['logo_file']['tmp_name'];
				$rnumber   = rand( 0, 999999 );
				$directory = QA_INCLUDE_DIR . 'watermark/';
				$NewImageName = $rnumber . '-' . basename( $ImageName );
				$path = $directory . $NewImageName;
				move_uploaded_file($TempSrc, $path);
				qa_set_option('logo_url', $NewImageName);
			}
			if (is_array(@$_FILES['night_logo_file']) && $_FILES['night_logo_file']['size']) {
				$ImageName = str_replace( ' ', '-', strtolower( $_FILES['night_logo_file']['name'] ) );
				$TempSrc   = $_FILES['night_logo_file']['tmp_name'];
				$rnumber   = rand( 0, 999999 );
				$directory = QA_INCLUDE_DIR . 'watermark/';
				$NewImageName = $rnumber . '-' . basename( $ImageName );
				$path = $directory . $NewImageName;
				move_uploaded_file($TempSrc, $path);
				qa_set_option('night_logo_url', $NewImageName);
			}
		}
	}
		...
		if (empty($optionname)) {
			...
			switch ($optionname) { // special treatment for certain options
				...
				case 'permit_post_wall':
					...
					if ( ($optionname=='permit_view_q_page') || ($optionname=='permit_post_q') || ($optionname=='permit_close_q') || ($optionname=='permit_post_a') || ($optionname=='permit_post_c') || ($optionname=='permit_anon_view_ips') )

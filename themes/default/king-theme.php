...
$this->output('<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
		...
		function header()
		{
            ...
            $this->output('</DIV>');	
			if ( $this->template == 'ask' || $this->template == 'news' ) {
			$this->imgupload();
    ...
    elseif ((qa_opt('avatar_allow_gravatar') || qa_opt('avatar_allow_upload')) && qa_opt('avatar_default_show') && !empty($defaultBlobId))  
		...
		if (!qa_opt('disable_image')) {
		$this->output('<a href="'.qa_path_html('ask').'" class="kingaddimg">'.qa_lang_html('main/home_image').'</a>');
		}
		if (!qa_opt('disable_video')) {	
		$this->output('<a href="'.qa_path_html('video').'" class="kingaddvideo">'.qa_lang_html('main/home_video').'</a>');
		}
		if (!qa_opt('disable_news')) {	
		$this->output('<a href="'.qa_path_html('news').'" class="kingaddnews">'.qa_lang_html('main/home_news').'</a>');
		}		
		$this->output('</div>');		
		$this->output('</div>');
		$this->output('</li>');
...
downloadCallback:function(filename,pd)
	{
		location.onclick=$(\'#q_content\').val("'.$mpageurl.'/king-include/uploads/'.$month_folder.'/thumb_"+filename),$(\'#king-editthumb\').attr({"src":"'.$mpageurl.'/king-include/uploads/'.$month_folder.'/thumb_"+filename+""});
	}
}); 
});
</script>');
...
downloadCallback:function(filename,pd)
	{
		location.onclick=$(\'#extra\').val($(\'#extra\').val()+"'.$mpageurl.'/king-include/uploads/'.$month_folder.'/"+filename+" "),$(\'#content\').val("'.$mpageurl.'/king-include/uploads/'.$month_folder.'/thumb_"+filename);
	}
}); 
});
</script>');
...
downloadCallback:function(filename,pd)
	{
		...
		function king_userpanel()
		{
			...
			$this->output('<a class="reglink" href="'.qa_path_html('logout').'">'.qa_lang_html('main/nav_logout').'</a>');
			}
			$this->output('</div>');

<?php
	class qa_html_theme extends qa_html_theme_base
	{
		...
		function body()
		{
			...
			$this->output('<script src="'.$this->rooturl.'jquery.sticky.js"></script>');
...
$this->output('<script>(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
		...
		function head_css()
		{
			...
			$this->output('<LINK REL="stylesheet" TYPE="text/css" HREF="'.$this->rooturl.'fonts/fonts.css"/>');
		...
		$this->output('<li class="king-nav-sub-item king-nav-sub-recent"><a href="'.qa_path_html('video').'" class="king-nav-sub-link"><i class="submitvideo"></i> Video</a></li>');
		}
		$this->output('</ul>');
		}
		...
		if (qa_opt('nav_unanswered')) {	
		$this->output('<li class="king-nav-sub-item king-nav-sub-recent"><span class="king-nav-sub-link king-nav-sub-selected"><i class="submitvideo"></i> Video</span></li>');
		}
		$this->output('</ul>');
		}
		}
        ...
        qa_db_user_account_selectspec($handle, false)
		    ...
		    if (qa_is_logged_in()) {
			$this->output('<DIV CLASS="king-leftacc">');
			$this->output('<li class="king-nav-main-item king-leftavatar">');
		...
		function header()
		{
			...
			$this->output('<DIV CLASS="king-nav-top">');
		...
		if (qa_opt('nav_unanswered')) {	
		$this->output('<a href="'.qa_path_html('video').'" class="kingaddvideo">Video</a>');
		}		
		} else {
		if (qa_opt('nav_ask')) {
		$this->output('<label for="checkbox-menu2" class="kingaddimg">Image</label>');
		}
		if (qa_opt('nav_unanswered')) {	
		$this->output('<label for="checkbox-menu2" class="kingaddvideo">Video</label>');
		}		
		}
		$this->output('</div>');
		$this->output('</li>');
...
downloadCallback:function(filename,pd)
	...
	ias.extension(new IASSpinnerExtension());
			...
			$types = array(
				...
				'facebook2'=>array(
					array(
						...
						'<div class="fb-video" width="'.$w.'" height="'.$h.'" data-allowfullscreen="true"  data-href="/$1/videos/$2/?type=1">'
					)
				),
		...
		function title2()
		{
		if (( $this->template == 'tag' ) || ( $this->template == 'tags' ) || ( $this->template == 'search' ) || ( $this->template == 'qa' ) ) {
		...
		{	
			...
			$this->output('<a  href="#" onclick="popUp=window.open(http://www.facebook.com/sharer.php?u='.qa_path_html(qa_q_request($this->content['q_view']['raw']['postid'], $this->content['q_view']['raw']['title']), null, qa_opt('site_url')).', popupwindow, scrollbars=yes,width=800,height=400);popUp.focus();return false">');
